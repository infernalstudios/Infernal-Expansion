...
public class PlantedQuartzBlock extends HorizontalBushBlock {
    ...
    public PlantedQuartzBlock(Properties properties) {
        super(properties);
    ...
    public BlockState getPlaceableState(World world, BlockPos pos, Direction placeSide) {
        if ((world.isAirBlock(pos) ||
                ...
                world.getBlockState(pos).getBlock() != IEBlocks.PLANTED_QUARTZ.get()) {
    ...
    protected boolean isValidGround(BlockState state, IBlockReader worldIn, BlockPos pos) {
        return
			state.matchesBlock(IEBlocks.GLOWDUST_SAND.get()) || state.matchesBlock(Blocks.SAND) || state.matchesBlock(Blocks.RED_SAND) ||
                ...
                state.matchesBlock(Blocks.NETHERRACK) || state.matchesBlock(Blocks.NETHER_QUARTZ_ORE) ||
				state.matchesBlock(Blocks.CRIMSON_NYLIUM) || state.matchesBlock(Blocks.NETHER_WART_BLOCK) ||

                state.matchesBlock(Blocks.NETHER_QUARTZ_ORE) || state.matchesBlock(Blocks.QUARTZ_BLOCK) ||
                state.matchesBlock(Blocks.QUARTZ_BRICKS) || state.matchesBlock(Blocks.SMOOTH_QUARTZ) ||
                state.matchesBlock(Blocks.QUARTZ_PILLAR) || state.matchesBlock(Blocks.CHISELED_QUARTZ_BLOCK) ||
