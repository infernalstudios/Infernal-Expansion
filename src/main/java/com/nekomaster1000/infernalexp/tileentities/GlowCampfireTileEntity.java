...
public class GlowCampfireTileEntity extends TileEntity implements IClearable, ITickableTileEntity {
    ...
    public void tick() {
        boolean flag = this.getBlockState().get(GlowCampfireBlock.LIT);
        ...
        if (world != null) {
            ...
            if (random.nextFloat() < 0.11F) {
                for(int i = 0; i < random.nextInt(2) + 2; ++i) {
                    GlowCampfireBlock.spawnSmokeParticles(world, blockpos, this.getBlockState().get(GlowCampfireBlock.SIGNAL_FIRE), false);
                }
            }

            int l = this.getBlockState().get(GlowCampfireBlock.FACING).getHorizontalIndex();
