...
public record PlantedQuartzFeatureConfig(float chanceToFail) implements FeatureConfiguration {

    public static final Codec<PlantedQuartzFeatureConfig> CODEC = RecordCodecBuilder.create((builder) ->
        builder.group(Codec.FLOAT.fieldOf("chance_to_fail").forGetter((config) -> config.chanceToFail))
