/*
 * Copyright 2021 Infernal Studios
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package org.infernalstudios.infernalexp.world.gen.features;

import com.mojang.serialization.Codec;
import net.minecraft.core.BlockPos;
import net.minecraft.world.level.block.Blocks;
import net.minecraft.world.level.levelgen.feature.Feature;
import net.minecraft.world.level.levelgen.feature.FeaturePlaceContext;
import net.minecraft.world.level.levelgen.feature.configurations.NoneFeatureConfiguration;
import org.infernalstudios.infernalexp.init.IEBlocks;
import org.infernalstudios.infernalexp.util.ShapeUtil;

import java.util.ArrayList;
import java.util.HashSet;
import java.util.List;
import java.util.Set;

// This has been replaced GlowstoneRavineCarver because of the size limitation of features

@Deprecated
public class GlowstoneRavineFeature extends Feature<NoneFeatureConfiguration> {
    public GlowstoneRavineFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }

    @Override
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        BlockPos endPos = context.origin().offset((40 + context.random().nextInt(20)) * (context.random().nextInt(2) == 0 ? -1 : 1), 0, (40 + context.random().nextInt(20)) * (context.random().nextInt(2) == 0 ? -1 : 1));

        if (!context.level().isEmptyBlock(context.origin()) || context.level().getBlockState(context.origin().below()) != IEBlocks.GLOWDUST_SAND.get().defaultBlockState() || context.level().getBlockState(endPos.below()) == Blocks.LAVA.defaultBlockState()) {
            return false;
        } else {
            List<BlockPos> centerLine = ShapeUtil.generateLine(context.origin(), endPos);
            List<BlockPos> sidePoints = new ArrayList<>();

            int segments = context.random().nextInt(3) + 1;
            System.out.println("Segments: " + segments);

            // Find the side points to create the outline
            for (int i = 1; i <= segments; i++) {
                BlockPos segment = centerLine.get(((centerLine.size() / 3) * i) - 1);

                sidePoints.add(segment.offset(1 + context.random().nextInt(2), 0, 1 + context.random().nextInt(2)));
                sidePoints.add(segment.offset((1 + context.random().nextInt(2)) * -1, 0, (1 + context.random().nextInt(2)) * -1));
            }

            Set<BlockPos> points = new HashSet<>();

            // Find outline of ravine
            for (int i = 0; i < segments; i++) {
                if (i == 0) {
                    points.addAll(ShapeUtil.generateLine(context.origin(), sidePoints.get(i)));
                    points.addAll(ShapeUtil.generateLine(context.origin(), sidePoints.get(i + 1)));
                } else if (i == segments - 1) {
                    points.addAll(ShapeUtil.generateLine(sidePoints.get(i), endPos));
                    points.addAll(ShapeUtil.generateLine(sidePoints.get(i + 1), endPos));
                } else {
                    points.addAll(ShapeUtil.generateLine(sidePoints.get(i), sidePoints.get(i + 2)));
                    points.addAll(ShapeUtil.generateLine(sidePoints.get(i + 1), sidePoints.get(i + 3)));
                }
            }

            // Fill in outline
            for (BlockPos centerPoint : centerLine) {
                for (BlockPos sidePoint : sidePoints) {
                    points.addAll(ShapeUtil.generateLine(centerPoint, sidePoint));
                }
            }

            // Replace blocks with air
            for (BlockPos point : points) {
                for (int y = 0; y < 10; y++) {
                    context.level().setBlock(point.below(y), Blocks.AIR.defaultBlockState(), 2);
                }
            }

            return true;
        }
    }
}
