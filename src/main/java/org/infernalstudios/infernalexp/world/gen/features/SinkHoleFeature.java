...
public class SinkHoleFeature extends Feature<NoFeatureConfig> {
    ...
    public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
        ...
        if (!world.isAirBlock(mutableBlockPos) || world.getBlockState(mutableBlockPos.move(Direction.DOWN)).getBlock() != Blocks.SAND) {
            return false;
        } else {
            ...
            for (BlockPos point : ShapeUtil.generateSolidCircle(radius + 1)) {
                ...
                for (int y = 0; y < 3; y++) {
                    world.setBlockState(mutableBlockPos.move(Direction.DOWN), Blocks.SAND.getDefaultState(), 2);
                }
            }
            ...
            for (BlockPos point : ShapeUtil.generateSolidCircle(radius)) {
                ...
                mutableBlockPos.move(point.getX(), point.getY(), point.getZ());
                mutableBlockPos.move(Direction.DOWN);
                world.setBlockState(mutableBlockPos, Blocks.GRAVEL.getDefaultState(), 2);
